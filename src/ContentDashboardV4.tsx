...
export default function ContentDashboardV4({ onBackToLanding, loadedPageId: _loadedPageId, savedPages: _savedPages, onUpdateSavedPages: _onUpdateSavedPages }: ContentDashboardV4Props) {
  ...
  const savePage = async () => {
    ...
    const currentPageSnapshot = normalizedPages[currentPageIndex] || currentPage;
    const coverPage = normalizedPages.find(p => p.pageNumber === 'cover');
    const pageName =
    ...
    const coverImage =
      coverPage
        ? ((coverPage.fields.showHeroImage !== false && coverPage.images.coverImage) 
            ? coverPage.images.coverImage 
    ...
    const coverData = coverPage ? {
      ...
      heroImage: coverPage.images.coverImage || null,
    ...
    const isEditingExisting = !!_loadedPageId && (_savedPages || []).some((p: any) => p.id === _loadedPageId);
    const original = isEditingExisting ? (_savedPages || []).find((p: any) => p.id === _loadedPageId) : null;
    const saved = {
      id: isEditingExisting ? _loadedPageId : `${Date.now()}`,
      name: pageName,
    ...
    const next = isEditingExisting
      ? (_savedPages || []).map((p: any) => (p.id === _loadedPageId ? saved : p))
      : [...(_savedPages || []), saved];
    try {
      const res = await saveDraft(saved);
      const drafts = await getDrafts();
      _onUpdateSavedPages?.(drafts.length ? drafts : next);
      alert(isEditingExisting ? 'Draft updated' : 'Page saved to Draft Library');
    } catch (e) {
                          ...
                          <div className="flex items-center justify-between mb-[8px]">
                            ...
                            <button
                              onClick={() => {
                                ...
                                const newBodies = currentPage.fields.bodyCopies?.filter(b => b.afterHeaderId !== header.id) || [];
                                setPages(prev => {
                                  const newPages = [...prev];
                                  newPages[currentPageIndex] = {
                                    ...newPages[currentPageIndex],
